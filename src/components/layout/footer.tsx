    ...
    <footer className="bg-charcoal text-cream">
      <div className="mx-auto max-w-7xl px-4 py-12">
        ...
        <div className="grid gap-10 sm:grid-cols-2">
          {LOCATIONS.map((loc) => (
            <div key={loc.id}>
              <h3 className="font-display text-lg font-bold mb-3">{loc.name}</h3>
              <p className="text-sm text-cream/70 mb-1">{loc.type}</p>

              <div className="mt-3 flex items-start gap-2 text-sm">
                <MapPin className="h-4 w-4 mt-0.5 shrink-0 text-cream/50" />
                <span className="text-cream/80">{loc.address}</span>
              </div>
              ...
              <a
                ...
                className="mt-3 inline-flex items-center gap-2 rounded-md bg-[#25D366] px-3 py-2 text-sm font-medium text-white hover:bg-[#25D366]/90 transition-colors min-h-12"
              >
                <MessageCircle className="h-4 w-4" />
                WhatsApp {loc.shortName}
              </a>

              <p className="mt-3 text-xs text-cream/50">{loc.hours}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-10 flex items-center gap-4 border-t border-cream/10 pt-6">
          <a
            ...
            rel="noopener noreferrer"
            aria-label="Facebook"
            className="hover:text-orange transition-colors min-h-12 min-w-12 flex items-center justify-center"
          >
        ...
        <div className="mt-6 text-xs text-cream/40 space-y-1">
          ...
          <p>
            &copy; {new Date().getFullYear()} {BRAND.legal} t/a {BRAND.name}
          </p>
        </div>
      </div>
    </footer>
